...
class _GraphAgentsState extends State<GraphAgents> {
  ...
  _buildNodes() {
    ...
    Map<String, Node> nodesTo = Map.fromIterable(widget.agents,
        key: (e) => e.getToWithDefault(),
        value: (e) {
          if (nodesFrom.containsKey(e.getToWithDefault())) {
            return nodesFrom[e.getToWithDefault()];
          } else {
            return Node(nodeText(e.getToWithDefault(), e));
          }
        });

    widget.agents.forEach((element) {
      if (element.to == null || element.to.isEmpty) {
        graph.addEdge(nodesFrom[element.from], nodesTo[element.getToWithDefault()], paint: Paint()..color = Colors.red);
      } else {
        graph.addEdge(nodesFrom[element.from], nodesTo[element.getToWithDefault()]);
      }
    });
